...
def display(request):
    try:
        conn = psycopg2.connect("dbname={} user={} password={} host={} port={}".format(
            ...
            settings.DATABASES['default']['PORT']))
        with conn.cursor() as curs:
            curs.execute("""select episode_nb, title, opening_crawl, director, producer, release_date, created, updated from ex06_movies""")
...
def update(request):
    ...
    try:
        conn = psycopg2.connect("dbname={} user={} password={} host={} port={}".format(
            ...
            settings.DATABASES['default']['PORT']))
        curs = conn.cursor()
        curs.execute("""select title from ex06_movies""")
        movies = curs.fetchall()
        if not movies:
            raise
